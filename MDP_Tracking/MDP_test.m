...
if is_kitti == 1
    ...
    opt.tracked = 5;  % change this to allow for shorter tracks
else
...
for fr = 1:seq_num
    ...
    if is_kitti
        ...
        ind = strcmp('car', dres.type) | strcmp('person', dres.type);
        index = find(ind == 1);
        dres = sub(dres, index);        
    end
...
if is_kitti == 0
    ...
    foldername = fullfile(opt.model_folder, model.save_folder, dataset.dataset_slug);
    if ~exist(foldername, 'dir')
        mkdir(foldername);
    end
    filename = fullfile(foldername, name);
    fprintf('write results: %s\n', filename);
    ...
    % save results
    if is_save
        folder_path = fullfile(opt.model_folder, model.save_folder, dataset.dataset_slug);
        filename = sprintf('%s/kitti_%s_%s_results.mat', folder_path, seq_set, seq_name);
...
for i = 1:num_track
    ...
    if isfield(dres_track, 'type')
        ...
        if strcmp(cls, 'person') == 1 || strcmp(cls, 'bicycle') == 1

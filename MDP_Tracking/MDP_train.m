...
%
% training MDP
function tracker = MDP_train(seq_idx, tracker, is_kitti, model, dataset)

if nargin < 3
    is_kitti = 0;  % is_kitti -> non-MDP stuff / any kitti format
end
...
if is_kitti == 0
    ...
    seq_num = dataset.sequence_lengths(seq_idx);

    dres_image = load_image_dres(opt.data_folder, seq_set, seq_name, seq_num, dataset);
    
    % generate training data
    I = dres_image.Igray{1};
    [dres_train, dres_det, labels] = generate_training_data_kitti( ...
        seq_idx, dres_image, opt, dataset);    
end
...
if nargin < 2 || isempty(tracker) == 1
    ...
    tracker.w_active = svmtrain(tracker.lactive, tracker.factive, '-c 1 -q');  % deb
end
...
while 1
    ...
    % Guard against having no good training sequences (@cyrus)
    if num_train == 0
        break
    end
    ...
    % first frame
    fr = dres_gt.fr(1); % assumes the frames are sorted first to last in kitti file
    id = dres_gt.id(1);
    ...
    while fr <= seq_num
        ...
        % inactive
        if tracker.state == 0
            if exist('reward') && reward == 1
                is_good(t) = 1;
                fprintf('sequence %d is good\n', t);
            elseif ~exist('reward')
                fprintf('REWARD DOES NOT EXIST. BYPASSING ERROR\n');
            end
            break;
        ...
        elseif tracker.state == 3
            ...
            if tracker.streak_occluded > opt.max_occlusion
                ...
                % need else; if not empty^, reward not set and will error out on state=0(@cyrus)
                % not sure what their algorithm specifies should be done here...
                if is_text
...
if is_save
    if is_kitti == 0
        ...
        save_tracker(opt.model_folder, model, seq_name, tracker);
    end
end